...
class Command(BaseCommand):
    help = 'Generate mock transactions with priority and status (Completed, Cancelled, Cut Off) only.'
    ...
    def handle(self, *args, **kwargs):
        ...
        if not verified_users:
            print("[Warning] No verified users found to assign as reservedBy.")
        else:
        ...
        def weighted_status_choice():
            return random.choices(
                ...
                weights=[85, 12, 3],  # Adjust probability here
                k=1
            )[0]
        ...
        def generate_for_model(model_class, count):
            ...
            if not population:
                print(f"[Warning] No data found for {model_class.__name__}")
                return

            for _ in range(count):
                requester = random.choice(population)

                # Set and persist priority
        ...
        print("✅ Done generating transactions with cutoff/completed/cancelled only and verified cashiers.")
